...
func resourceNetworkACLRead(d *schema.ResourceData, meta interface{}) error {
	...
	err := resource.Retry(tfec2.NetworkACLPropagationTimeout, func() *resource.RetryError {
		var err error

		networkAcl, err = tfec2.FindNetworkACLByID(conn, d.Id())
	...
	tags := tftags.Ec2KeyValueTags(networkAcl.Tags).IgnoreAws().IgnoreConfig(ignoreTagsConfig)
